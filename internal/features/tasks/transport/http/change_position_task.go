...
type ChangePositionTaskRequest struct {
	Position int `json:"position" example:"2"`
}

func (r *ChangePositionTaskRequest) Validate() error {
	if r.Position < 1 {
...
func (h *TasksHTTPHandler) ChangePositionTask(
	...
	var request ChangePositionTaskRequest
	...
	task, err := h.tasksService.ChangePosition(
		ctx, userID, taskID, request.Position,
	)
	if err != nil {
		responseHandler.ErrorResponse(
			err,
			"failed to change task position",
		)
		return
	}
